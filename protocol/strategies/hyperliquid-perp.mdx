...
| **Sweep** | `sweepToVault()` pushes any latecomer EVM USDC arrivals to the vault. Callable by anyone; repeatable for partial arrivals. | Anyone |
| **Recover HC residuals** (post-settle) | `recoverHcResiduals()` re-fires the full HC drain (cancel + force-close + perp→spot + spot→EVM bridge). Use after IOC slippage left residual perp margin, or after funding the strategy's HC HYPE balance to retry a bridge that previously no-op'd. Permissionless; repeatable. After it fires, call `sweepToVault()` once HC delivers the bridged USDC on EVM. | Anyone |

<Warning>
**Proposer must fund the strategy with HYPE on HyperCore before `initiateReturn`.** The spot→EVM `sendAsset` action consumes HC-side gas (HYPE), per [hyper-evm-lib's `bridgeToEvm` note](https://github.com/hyperliquid-dev/hyper-evm-lib/blob/main/src/CoreWriterLib.sol). If the strategy's HC HYPE balance is zero, the bridge action silently no-ops on HC and USDC stays on HC spot indefinitely. Recommended: send a small amount of HYPE (≤0.01 HYPE is sufficient for one bridge) to the strategy's HC address right after the clone is deployed and before `_execute` runs. Recovery from a HYPE-less stranded state: fund HYPE on HC, then call public `recoverHcResiduals()` post-settle (or `initiateReturn()` pre-settle) to retry the spot→EVM leg.
</Warning>
