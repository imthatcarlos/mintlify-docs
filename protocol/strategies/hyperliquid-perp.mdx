...
|-------|-------------|-----------|
| **Execute** | Pull USDC from vault → `sendUsdClassTransfer` to perp margin (leverage is set off-chain by the keeper via `updateLeverage` API call before the proposal opens — Hyperliquid's CoreWriter has no on-chain leverage action) | Governor (proposal execution) |
