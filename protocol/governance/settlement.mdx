...
**Redemption lock:** When a strategy is live (Executed state), vault redemptions (`withdraw` / `redeem`) are blocked. A WOOD/SHARES liquidity pool for early exit is planned for a future release (see [Economics](/protocol/governance/economics)).
