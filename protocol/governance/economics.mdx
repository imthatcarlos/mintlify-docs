...
When a strategy is live, vault redemptions are blocked. WOOD/SHARES liquidity pools will provide an early exit mechanism, pairing the WOOD protocol token with each vault's share token. Depositors who want out during strategy execution will be able to sell their shares into the pool, while buyers get discounted exposure to the active strategy outcome.
